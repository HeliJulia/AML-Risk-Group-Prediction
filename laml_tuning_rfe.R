...
# Predicting Risk Category from Genes
# LAML MODEL TUNING
# Heli Leskelä
# University of Oulu
# July 2022
...
# Loading the subset of features -----------------------------------------------
rfe <- read.csv("rfe.txt", sep="\t")
subset <- rfe$variables[which.max(rfe$accuracy)]
subset <- unlist(strsplit(subset, split = ", "))

write.table(subset, file = "important_genes.txt", sep = "\t", row.names = FALSE)


# Selecting only the subset of features ----------------------------------------
train %<>%
  select(c(y, subset)) %>% mutate(y = as.factor (y)) 
test %<>%
  select(c(y, subset)) %>% mutate(y = as.factor (y))
...
poor_adas <- ADAS(X = norm_poor[,-1], target = norm_poor$y, K = 10)$syn_data
fav_adas <- ADAS(X = norm_fav[,-1], target = norm_fav$y, K = 25)$syn_data
fav_adas2 <- ADAS(X = norm_fav[,-1], target = norm_fav$y, K = 25)$syn_data
...
config_set$units <- length(subset)
config_set %<>% distinct()
...
tune.network <- function(config_set, epochs, x_train, y_train){
  ...
  x <- c("layers", "units", "drop", "lr", "batch_size")
...
results %>%
  mutate(method = rep("RFE+SMOTE", length(models$confusion_matrix))) %>%
...
#write.table(results, file = "tuning_models_rfe.txt", sep = "\t", row.names = FALSE)
write.table(results, file = "tuning_models_rfe.txt", sep = "\t", append = TRUE,
            row.names = FALSE, col.names = FALSE)


# Loading the saved models, and saving the best ones ---------------------------
models_df <- read.csv("tuning_models_rfe.txt", sep="\t")

models_df %>%
  arrange(desc(accuracy)) %>%
  select(layers, units, drop, lr, batch_size, method,
         accuracy, f1_fav, f1_norm, f1_poor) -> best_a_rfe
write.table(best_a_rfe[1:10,], file = "best_a_rfe.txt", sep = " & ", row.names = FALSE)

models_df %>%
  arrange(desc(f1_fav)) %>%
  select(layers, units, drop, lr, batch_size, method,
         accuracy, f1_fav, f1_norm, f1_poor) -> best_fav_rfe
write.table(best_fav_rfe[1:10,], file = "best_fav_rfe.txt", sep = " & ", row.names = FALSE)

models_df %>%
  arrange(desc(f1_norm)) %>%
  select(layers, units, drop, lr, batch_size, method,
         accuracy, f1_fav, f1_norm, f1_poor) -> best_norm_rfe
write.table(best_norm_rfe[1:10,], file = "best_norm_rfe.txt", sep = " & ", row.names = FALSE)

models_df %>%
  arrange(desc(f1_poor)) %>%
  select(layers, units, drop, lr, batch_size, method,
         accuracy, f1_fav, f1_norm, f1_poor) -> best_poor_rfe
write.table(best_poor_rfe[1:10,], file = "best_poor_rfe.txt", sep = " & ", row.names = FALSE)
